...
class SurveyContacts(Survey):
    ...
    def get_contact_modal(self, survey_token, answer_token):
        """
        Return rendered modal HTML for JS
        """
        ...
        if answer_sudo and request.env.user.partner_id in answer_sudo.contact_ids:
            return (
                ...
                .sudo()
                ._render_template(
                    "survey_contact_ids.invite_contacts_modal",
                    {
    ...
    @http.route(
        "/get/invites/<string:survey_token>/<string:answer_token>",
        type="json",
        auth="user",
        website=True,
    )
    def get_invites_modal(self, survey_token, answer_token):
        """
        Return rendered modal HTML for JS
        """
        answer_from_cookie = False
        ...
        if answer_sudo and request.env.user.partner_id in answer_sudo.contact_ids:
            return (
                ...
                .sudo()
                ._render_template(
                    "survey_contact_ids.show_invites_modal",
                    {
                        "answer_token": answer_token,
                        "survey_token": survey_token,
                        "answer": answer_sudo,
    ...
    def survey_contacts_post(self, survey_token, answer_token, **post):
        response = {"error": False, "msg": _("Contact added successfully.")}
    ...
    def survey_invite_accept(self, code):
        ...
        return request.redirect(f"/my/surveys/{invitation.survey_user_input_id.id}")
